...
open(IN, "$ARGV[0]");
open(OUTALL, ">$ARGV[0]-alt20orMore-all");
#open(OUTMISS, ">$ARGV[0]-alt20orMore-invar");
#open(OUTKEEP, ">$ARGV[0]-alt20orMore-keep");
my @line;
my $ref;
my $alt;
my $reffreq;
    ...
    if ($i < 9){
      ...
      my @geno = split(/:/,$line[$i]);
        if(defined($geno[1])){
            ...
            if(sum(@depth)<10){push(@outline, "N"); $missing++;}
            elsif((($depth[1]/($depth[0]+$depth[1]))*100)>=20){push(@outline, "$alt "); $altfreq++;}
            elsif($depth[0]>$depth[1]){push(@outline, "$ref "); $reffreq++;}
